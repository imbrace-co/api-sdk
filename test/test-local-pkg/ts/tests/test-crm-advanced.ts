...
async function testCrmAdvanced() {
  ...
  try {
    ...
    await runTestSection("Board Relations (Link/Unlink)", async () => {
      ...
      // data-board wraps in {data: ...}, legacy backend returned bare object
      state.boardId = (board1 as any).data?._id || (board1 as any).data?.id || board1._id || (board1 as any).id;
      state.relatedBoardId = (board2 as any).data?._id || (board2 as any).data?.id || board2._id || (board2 as any).id;

      const item1 = await client.boards.createItem(state.boardId!, { fields: {} });
      const item2 = await client.boards.createItem(state.relatedBoardId!, { fields: {} });
      state.itemId = (item1 as any).data?._id || (item1 as any).data?.id || item1._id || (item1 as any).id;
      state.relatedItemId = (item2 as any).data?._id || (item2 as any).data?.id || item2._id || (item2 as any).id;

      const linkRes = await client.boards.linkItems(state.boardId!, state.itemId!, state.relatedBoardId!, {
          relatedItemIds: [state.relatedItemId!]
      });
