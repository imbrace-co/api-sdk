---
title: CLI 命令
description: 所有 Imbrace CLI 命令的完整参考。
---
...
AI 代理是一个配置好的助手（LLM + 提示词 + 行为）。创建时会原子性地预配助手、Web 渠道和用例模板。
...
`create` 和 `update` 接受相同的行为标志。`update` 通过 PUT-merge 保留未更改的字段。`--id-only` 仅适用于 `create`。
...
Document AI 代理从非结构化文档（PDF、图片、扫描表单）中提取结构化 JSON。每个代理都有一个定义提取字段的 **schema**、指导 LLM 的**指令**以及**模型 + 提供商**。存储为 AI 代理，`agent_type: "document_ai"`。
...
`list` 默认按 `documentAiOnly: true` 过滤。传入 `--all` 以包含常规代理。
...
CLI 会自动规避以下三种平台行为，调用方无需了解 — 但在调试原始 SDK 响应时了解它们会很有帮助：
...
> `@imbrace/cli` `v0.8.0`+。请勿与上文的 `imbrace workflow mcp ...` 混淆 — 那组命令管理的是工作流引擎自身的 MCP-server 资源。想零安装？网关通过 HTTP 托管同一套工具 — 参见 [MCP Server](/zh-cn/mcp/overview/)。

`imbrace mcp` 通过 **stdio** 运行一个 [Model Context Protocol](https://modelcontextprotocol.io) 服务器，把 Imbrace API 作为工具暴露给 AI 代理（Claude Code、Claude Desktop、Cursor 等）。工具与 SDK 的 [`client.api`](/sdk/overview/#the-full-api-surface--clientapi-typescript) 表面出自同一套 `agent-tools` OpenAPI 规范 — 5 个服务共 272 个操作 — 命名格式为 `<service>_<operationId>`，例如 `dataBoard_listBoards`、`workflow_listFlows`。

```bash
imbrace mcp                                     # 只读工具（默认）
imbrace mcp --allow-write                       # + 创建/修改数据的操作
imbrace mcp --allow-write --allow-destructive   # + 删除数据的操作
imbrace mcp --services dataBoard,workflow       # 限定服务
imbrace mcp --tags boards,records               # 按 OpenAPI 标签限定
imbrace mcp --profile dev                       # 使用指定 profile 运行
```
...
| 默认（只读） | 140 |
| `--allow-write` | 229 |
| `--allow-destructive`（隐含 `--allow-write`） | 272 |

服务：`dataBoard`、`channel`、`platform`、`marketplace`、`workflow`。

**请先登录。** stdout 是 JSON-RPC 通道，因此命令会直接报错而不是交互式提示：
...
管理多个凭证 profile（AWS 风格）。每个 profile 存储凭证、环境预设、组织 ID 以及其他 SDK 设置。解析顺序：`--profile` 参数 > `IMBRACE_PROFILE` 环境变量 > `active_profile` 配置 > `"default"`。

### 创建

创建新的 profile，并向平台验证凭证：
...
`--force` 跳过凭证验证。如果该 profile 已存在，命令会报错。
...
默认显示当前激活的 profile。显示所有已存储的字段（凭证预览、登录方式、邮箱、环境、Base URL、组织 ID、超时、服务、健康检查）。

### 切换

切换后续所有命令使用的激活 profile：
...
打印捆绑的 `llms.txt` — 一个完整的命令参考，专为提供给编码代理作为上下文而设计（Claude、Cursor 等）。
...
将 CLI 自动更新到 npm 上的最新版本。

```bash
imbrace update              # 更新到最新版本
imbrace update --check      # 检查更新但不安装
imbrace update --version 0.6.5  # 安装指定版本
imbrace update --json       # 机器可读的 JSON 输出
```
...
<Aside type="note">
  `imbrace update` 从 **v0.6.8** 起可用。旧版本用户需先手动运行 `npm install -g @imbrace/cli@latest` 一次以获得此命令。
</Aside>
