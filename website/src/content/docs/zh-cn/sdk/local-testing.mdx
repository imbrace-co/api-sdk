...
同一测试会分别使用 `IMBRACE_API_KEY` 和 `IMBRACE_ACCESS_TOKEN` 运行，以覆盖两种认证模式。
...
**dist 文件报 `ERR_MODULE_NOT_FOUND`**（TypeScript）
...
- [测试指南 →](/guides/testing/) — SDK 源码的单元测试、集成测试、代码检查和类型检查
- [错误处理 →](/sdk/error-handling/) — SDK 如何呈现错误和进行重试
- [完整流程指南 →](/sdk/full-flow-guide/) — 回归测试所对应的端到端演练
