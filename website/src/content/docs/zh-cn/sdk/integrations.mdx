...
两个 SDK 的框架级接入模式。选择适合你技术栈的部分 — TypeScript 涵盖 React、Next.js 和纯 Node.js；Python 涵盖 FastAPI、asyncio、Django 和 Celery。两个 SDK 的 OTP 登录流程均有说明。
...
<Aside type="tip">
  不要跨 Celery 工作器共享单个 `ImbraceClient` 实例 — 使用上下文管理器在每个任务调用中创建一个。httpx 连接池不适合跨进程共享。
</Aside>
...
- [认证 →](/sdk/authentication/) — OTP 登录流程、凭证生命周期和令牌存储
- [错误处理 →](/sdk/error-handling/) — `AuthError`、`ApiError` 和 `NetworkError` 如何呈现
- [快速入门 →](/sdk/quick-start/) — 添加框架接入之前的最小可运行示例
