---
title: Document AI
description: 使用视觉模型从 PDF 和图像中提取结构化数据。两个命名空间：client.chatAi 用于直接处理，client.documentAi 用于基于代理的工作流。
---
...
- **`client.documentAi` / `client.document_ai`** — 基于代理的文档处理，支持 schema 管理、代理增删改查和端到端编排
...
Document AI 代理存储提取 schema、指令和模型配置，供重复使用。
...
// 仅 Document AI 代理（通过 createFull 或 webapp 创建）
...
## `client.documentAi` — 建议 schema

通过分析示例文档自动提出 JSON schema。
...
端到端 Document AI 代理创建（iMBRACE webapp 所做的）：创建一个带有提取 schema 的面板，然后创建一个链接到该面板的 UseCase + AI 代理。
...
print(result["ai_agent_id"])   # 所创建的 AI 代理的 UUID
```
</TabItem>
</Tabs>
...
| Schema 字段 | `schemaFields` | `schema_fields` | `CreateBoardFieldInput[]` / `List[Dict]` | — |
...
- [完整流程指南 §3 — 知识中心](/zh-cn/sdk/full-flow-guide/#3-manage-knowledge-hubs-and-attach-to-an-ai-agent) — 上传用于 RAG 的文件
- [AI 代理 — 嵌入与知识库](/zh-cn/sdk/ai-agent/#embeddings--knowledge-base) — 管理用于检索的嵌入文件
