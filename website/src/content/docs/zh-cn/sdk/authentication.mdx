...
1. **`requestOtp` → `loginWithOtp`** 颁发短期的 `login_acc_...` 令牌（3 小时 TTL）。此令牌只能做一件事：列出用户所属的组织。
...
<Tabs syncKey="lang">
  <TabItem label="TypeScript">
    ...
    const chosen = organizations[0]  // ← 由你的 UI 选择
  ...
  <TabItem label="Python">
    ...
    # 步骤 2：用户提交 OTP。login_with_otp 会存储短期 login_acc_ 令牌
    #         并返回用户所属的组织。
    res = client.login_with_otp("user@example.com", "123456")
    chosen = res["organizations"][0]  # ← 由你的 UI 选择

    # 步骤 3：将 login_acc_ 兑换为绑定组织的 acc_ 令牌。
    #         SDK 会自动更新令牌 + x-organization-id。
...
`login()` 在单次调用中同时返回登录令牌和用户的组织列表。使用 `selectOrganization()` 选择其中一个，SDK 会为你替换令牌和组织请求头。
...
- [设置指南 →](/getting-started/setup/#configure-credentials) — 环境变量、dotenv 和密钥管理
- [快速入门 →](/sdk/quick-start/) — 使用你的凭证发起第一个 API 调用
- [集成 →](/sdk/integrations/) — 框架接入模式（React、Next.js、FastAPI、Django）
