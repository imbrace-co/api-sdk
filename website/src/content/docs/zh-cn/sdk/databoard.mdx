---
title: 数据面板
description: client.boards 的 SDK 参考文档 — CRM 管道、条目、字段、分段、知识中心、导入/导出和外部驱动器集成。
---
...
冲突现在直接在响应中体现：`updateItem` 在记录自调用方上次读取后被修改时返回 HTTP `409 Conflict` — 捕获由此产生的 `ApiError`（`statusCode === 409`）即可，无需再调用单独的端点。
...
// 链接和取消链接（data-board 服务 — POST/DELETE /boards/:id/items/:itemId/related）
