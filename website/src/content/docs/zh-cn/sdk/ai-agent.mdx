...
- **`chatAi` / `chat_ai`** — 创建和管理 AI 代理、文档/文件处理以及 Document AI。
...
// 创建。省略时 SDK 会自动填充 { provider_id: "system", model_id: "Default" }；
// "Default" 会路由到该组织配置的默认模型，可跨组织通用。像 "gpt-4o" 这样的
// 具体名称只有在该组织的 system 提供商确实公开它时才有效 —
// `client.ai.listModels()` 可以查看有哪些可用模型。传入 `streaming: true`，
// 这样 /v2/chat 会发出 text-delta SSE 事件，供 `streamChatText`（见下文）解析；
// 否则服务器会以单个 text/plain 响应返回完整回复，SSE 解析器将得不到任何内容。
...
# "Default" 会路由到该组织配置的默认模型，可跨组织通用。像 "gpt-4o" 这样的
# 具体名称只有在该组织的 system 提供商确实公开它时才有效 —
# `client.ai.list_models()` 可以查看有哪些可用模型。传入 `streaming: True`，
# 这样 /v2/chat 会发出 text-delta SSE 事件，供 `stream_chat_text`（见下文）解析；
# 否则服务器会以单个 text/plain 响应返回完整回复，SSE 解析器将得不到任何内容。
...
使用视觉模型从 PDF 和图片中提取结构化数据。完整参数参考、PDF 表单填写和异步用法请参见[Document AI 参考](/zh-cn/sdk/document-ai/)。
...
验证自定义工具服务器 URL 是否可达并返回有效的工具 schema。
...
`streamChat` / `stream_chat` 通过 `POST /ai-agent/v2/chat` 返回原始 `Response`。响应体的格式取决于代理的 `streaming` 标志：

| 代理 `streaming` | `Content-Type` | 响应体 |
|---|---|---|
| `true`（推荐） | `text/event-stream` | Vercel-AI-SDK UI-Message-Stream：`data: {"type":"text-delta","delta":"..."}` 事件 |
| `false`（新建代理的默认值） | `text/plain` | 以单个字符串返回的完整回复 — 没有 SSE 帧 |

### 推荐：`streamChatText` / `stream_chat_text`

这些便捷辅助方法封装了 `streamChat`，解析 SSE 事件流，并逐个产出纯文本增量。它们假定代理在创建时设置了 `streaming: true`。
...
<Aside type="caution" title="回复为空？请检查代理的 streaming 标志。">
  如果服务器返回 200 OK，但 `streamChatText` / `stream_chat_text` 产出 **0 个 chunk**，那么几乎可以肯定该代理设置了 `streaming: false`。此时响应是 `text/plain`，不包含任何 `data:` 行，SSE 解析器会静默地什么也不输出。解决方法：在创建时设置 `streaming: true`（或通过 `updateAiAgent(id, { streaming: true })` 设置），或者使用下方能识别 content-type 的模式来同时处理两种格式。
</Aside>

### 同时处理流式和非流式代理

当你无法控制代理的创建方式时，请根据 `Content-Type` 分支处理：
...
if (contentType.includes("text/event-stream")) {
  ...
  for await (const chunk of client.aiAgent.streamChatText(body)) {
    reply += chunk;
    process.stdout.write(chunk);   // 边接收边渲染
  }
} else {
  // 非流式代理 — 响应体即完整回复
...
分析示例文档（PDF/图片），为 Document AI 代理建议合适的 JSON schema。
...
聊天客户端子 API 为前端应用提供支持（例如嵌入式聊天小部件）。关于框架级接入（React 单例、Express 认证代理等），请参见[集成](/zh-cn/sdk/integrations/)。
