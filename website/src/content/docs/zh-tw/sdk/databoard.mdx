---
title: 資料看板
description: client.boards 的 SDK 參考文件 — CRM 管道、項目、欄位、區隔、Knowledge Hub、匯入/匯出和外部雲端硬碟整合。
---
...
請先初始化用戶端（請參閱[安裝](/sdk/installation/)或[快速入門](/sdk/quick-start/)）。
...
衝突檢查現在會直接在回應中回報：`updateItem` 在記錄自上次讀取後被修改時回傳 HTTP `409 Conflict` — 改成捕捉 `ApiError`（`statusCode === 409`），不需要另呼叫專用端點。
...
// 連結與解除連結（data-board 服務 — POST/DELETE /boards/:id/items/:itemId/related）
