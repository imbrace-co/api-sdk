---
title: Data Boards
description: client.boards 的 SDK 參考文件 — CRM 管道、項目、欄位、區段、Knowledge Hub、匯入/匯出和外部磁碟整合。
---
...
衝突現在內聯回報：`updateItem` 在記錄自上次讀取後被修改時回傳 HTTP `409 Conflict` — 改成捕捉 `ApiError`（`statusCode === 409`），不需要另呼叫專用端點。
