...
兩個 SDK 的框架層級整合模式。選擇您技術堆疊適用的章節 — TypeScript 涵蓋 React、Next.js 和純 Node.js；Python 涵蓋 FastAPI、asyncio、Django 和 Celery。兩個版本都有記錄 OTP 登入流程。

有關憑證策略（API 金鑰與存取權杖、環境變數），請參閱[認證](/sdk/authentication/)和[設定指南](/getting-started/setup/#configure-credentials)。
...
在元件樹外部建立用戶端一次，並在所有元件中重複使用。`localStorage` 中的權杖來自 [OTP 登入流程](/sdk/authentication/#otp-login-flow)。
...
最簡單的模式 — 每個請求一個非同步用戶端，生命週期由相依項管理：
...
為了獲得更高的吞吐量，可以在應用程式的整個生命週期中共用一個用戶端：
...
<Aside type="tip">
  請勿在 Celery 工作者之間共用單一 `ImbraceClient` 實例 — 請使用 context manager 在每次任務呼叫時建立一個。httpx 連線池無法安全地跨處理序共用。
</Aside>
...
OTP 流程在兩個 SDK 中的概念相同：請求一個 OTP 到電子郵件，然後將其兌換為存取權杖。請參閱[認證 → OTP 登入流程](/sdk/authentication/#otp-login-flow)以了解完整的憑證生命週期。
...
async def verify_otp(body: OtpVerify):
    async with AsyncImbraceClient() as client:
        try:
            result = await client.login_with_otp(body.email, body.otp)
...
- [認證 →](/sdk/authentication/) — OTP 登入流程、憑證生命週期與權杖儲存
- [錯誤處理 →](/sdk/error-handling/) — `AuthError`、`ApiError` 與 `NetworkError` 如何呈現
- [快速入門 →](/sdk/quick-start/) — 在加入框架串接前的最小可運作範例
