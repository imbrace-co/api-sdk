---
title: 錯誤處理
description: 錯誤類型、自動重試行為以及使用 Imbrace SDK 處理故障的最佳做法。
---

import { Tabs, TabItem } from "@astrojs/starlight/components";

所有 SDK 拋出的錯誤都繼承自單一基礎類型，因此您可以在一個地方捕獲任何 SDK 錯誤，或在需要不同處理方式時依特定子類別分別處理。TypeScript 和 Python SDK 的階層完全相同。
...
在建立用戶端時設定逾時（預設：**30 秒**）。當逾時觸發時，進行中的請求將被中止並拋出/引發 `NetworkError`。
