---
title: 完整流程指南
description: 端到端逐步解說 — 從用戶端設定到 AI Agent、工作流程、知識中心和資料看板。TypeScript 和 Python。
---
...
2. **建立 AI agent**
   ...
   `provider_id` 和 `model_id` 是可選的 — SDK 在省略時會自動填入 `{ provider_id: "system", model_id: "Default" }`。`"Default"` 路由到組織設定的系統預設模型，是跨組織通用的選擇。顯式傳入即可覆寫：`provider_id: "<uuid>"` 指定自訂 LLM 提供者，或 `model_id: "gpt-4o"` 等 — 但具體模型名稱只在該組織的 `system` 提供者實際公開該模型時才有效。在寫死於程式碼前，使用 `client.ai.listModels()`（TypeScript）/ `client.ai.list_models()`（Python）列出組織的可用模型。
...
1. **取得您的專案 ID**
   ...
   <Tabs syncKey="lang">
     <TabItem label="TypeScript">
       ```typescript
       // 選項 A — 從 Dashboard 取得後寫死（建議新組織使用）
...
3. **新增 Webhook 觸發條件並發佈流程**
   ...
   新建立的流程處於 **DRAFT**（草稿）狀態且沒有觸發條件 — Webhook URL 還不存在，因此 `triggerFlow` 會回傳 404。新增 Webhook 元件作為觸發條件，然後發佈：
...
3. **將資料夾附加至 AI agent**
   ...
   在 `folder_ids` 中傳遞資料夾 `_id` — AI agent 從該資料夾中的每個檔案檢索。使用 `board_ids` 來附加 CRM data-board（其項目成為知識來源 — 請參閱[資源 → Boards 與項目](/sdk/databoard/#items)）。舊的 `knowledge_hubs` 欄位已棄用。
