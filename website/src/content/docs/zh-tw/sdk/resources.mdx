---
title: 資源參考
description: Imbrace SDK 依命名空間的參考文件 — 聯絡人、對話、訊息傳遞、行銷活動等。
---

import { Tabs, TabItem } from "@astrojs/starlight/components";

此頁面涵蓋 SDK 公開的 CRM 和訊息傳遞命名空間。請先初始化用戶端（請參閱[安裝](/sdk/installation/)或[快速入門](/sdk/quick-start/)）。以下所有程式碼片段假設 `client` 是已初始化的實例。

有關 AI 和工作流程相關的資源，請參閱：
- [AI Agent](/sdk/ai-agent/) — AI agent（`chatAi`）、AI agent 串流、嵌入、parquet
- [工作流程](/sdk/workflows/) — 工作流程的流程（flow）與頻道自動化
