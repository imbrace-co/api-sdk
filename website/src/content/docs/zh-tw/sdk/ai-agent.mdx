---
title: AI 代理
description: chatAi / chat_ai（AI Agent 管理）、aiAgent / ai_agent（串流聊天、嵌入、parquet、追蹤）以及與 OpenAI 相容的 client.ai 服務的參考文件。
---
...
- **`aiAgent` / `ai_agent`** — 串流聊天（SSE）、嵌入管理、Parquet 資料、分散式追蹤，以及聊天用戶端子 API。
...
同步（`ImbraceClient`）和非同步（`AsyncImbraceClient`）用戶端都公開相同的方法表面 — 非同步方法需要 await，用戶端在底層使用 `AsyncAiAgentResource`。

---

## AI 代理 — `chatAi` / `chat_ai`

管理 AI agent（CRUD）、執行相容 OpenAI 的補全，以及負責文件/檔案處理。同一個命名空間也涵蓋知識中心資料夾和知識庫。
...
// 建立。省略時 SDK 會自動填入 { provider_id: "system", model_id: "Default" }；
// "Default" 會導向組織設定的預設模型，可跨組織通用。像 "gpt-4o" 這類
// 字面名稱只有在該組織的 system 提供者確實公開該模型時才有效 —
// `client.ai.listModels()` 會列出可用的模型。請傳入 `streaming: true`，讓
// /v2/chat 發出 `streamChatText`（見下文）可解析的 text-delta SSE 事件；
// 若未設定，伺服器會以單一 text/plain 回應傳回完整回覆，SSE 解析器將什麼也拿不到。
...
# "Default" 會導向組織設定的預設模型，可跨組織通用。像 "gpt-4o" 這類
# 字面名稱只有在該組織的 system 提供者確實公開該模型時才有效 —
# `client.ai.list_models()` 會列出可用的模型。請傳入 `streaming: True`，讓
# /v2/chat 發出 `stream_chat_text`（見下文）可解析的 text-delta SSE 事件；
# 若未設定，伺服器會以單一 text/plain 回應傳回完整回覆，SSE 解析器將什麼也拿不到。
...
過濾 AI agent 的輸出，以符合安全、政策合規或品牌規範的要求。
...
`streamChat` / `stream_chat` 會透過 `POST /ai-agent/v2/chat` 回傳原始 `Response`。其 body 格式取決於代理的 `streaming` 旗標：

| 代理 `streaming` | `Content-Type` | Body |
|---|---|---|
| `true`（建議） | `text/event-stream` | Vercel-AI-SDK UI-Message-Stream：`data: {"type":"text-delta","delta":"..."}` 事件 |
| `false`（新建立代理的預設值） | `text/plain` | 以單一字串傳回完整回覆 — 不含 SSE 事件格式 |

### 建議做法：`streamChatText` / `stream_chat_text`

這些便利輔助方法包裝了 `streamChat`，會解析 SSE 事件串流並逐一產出純文字差異片段。它們假設代理是以 `streaming: true` 建立的。
...
<Aside type="caution" title="回覆是空的？請檢查代理的 streaming 旗標。">
  如果伺服器回傳 200 OK，但 `streamChatText` / `stream_chat_text` 產出 **0 個片段**，幾乎可以確定代理設定為 `streaming: false`。此時回應為 `text/plain`、不含任何 `data:` 行，SSE 解析器便會靜默地什麼也不輸出。請在建立時設定 `streaming: true`（或透過 `updateAiAgent(id, { streaming: true })`）來修正，或使用下方依 content-type 分流的模式同時處理兩種格式。
</Aside>

### 同時處理串流與非串流代理

當你無法控制代理的建立方式時，請依 `Content-Type` 分流：
...
if (contentType.includes("text/event-stream")) {
  ...
  for await (const chunk of client.aiAgent.streamChatText(body)) {
    reply += chunk;
    process.stdout.write(chunk);   // 邊收到邊呈現
  }
} else {
  // 非串流代理 — body 即為完整回覆
...
分析範例文件（PDF/圖片），建議適用於 Document AI 代理的 JSON 結構。
...
聊天用戶端子 API 為前端應用程式（例如嵌入式聊天元件）提供支援。有關框架層級的整合（React singleton、Express 認證代理等），請參閱[整合](/sdk/integrations/)。
