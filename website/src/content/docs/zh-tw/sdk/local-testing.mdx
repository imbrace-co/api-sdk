...
本指南讓您測試**已建置的套件**，完全按照使用者安裝的方式進行 — 而非未建置的原始碼。在發佈新版本之前，或在重現使用者回報的錯誤時使用。

## 先決條件

- TypeScript：Node 18+
- Python：Python 3.9+ 和 `pip`
...
| 變數 | 取得位置 |
| ----------------- | --------------------------------------------------------------------------------------------- |
| `IMBRACE_API_KEY`     | Imbrace 入口網站，或使用現有存取權杖 `POST /private/backend/v1/third_party_token` |
...
- [測試指南 →](/guides/testing/) — SDK 原始碼的單元測試、整合測試、lint 與型別檢查
- [錯誤處理 →](/sdk/error-handling/) — SDK 如何呈現錯誤與重試
- [完整流程指南 →](/sdk/full-flow-guide/) — 回歸測試所對應的端對端逐步說明
