...
| `channel_type` | string? | 活動執行所在的頻道類型 |
...
|--------|-----------|--------|------|
| 列出行銷活動 | `list` | `list` | 列出所有活動 |
| 取得行銷活動 | `get` | `get` | 依 ID 取得活動 |
| 建立行銷活動 | `create` | `create` | 建立新活動 |
| 刪除行銷活動 | `delete` | `delete` | 刪除活動 |

### 接觸點

| 方法 | TypeScript | Python | 說明 |
|--------|-----------|--------|------|
| 列出接觸點 | `listTouchpoints` | `list_touchpoints` | 列出所有接觸點 |
| 取得接觸點 | `getTouchpoint` | `get_touchpoint` | 依 ID 取得接觸點 |
| 建立接觸點 | `createTouchpoint` | `create_touchpoint` | 向活動新增接觸點 |
| 更新接觸點 | `updateTouchpoint` | `update_touchpoint` | 更新接觸點 |
| 刪除接觸點 | `deleteTouchpoint` | `delete_touchpoint` | 移除接觸點 |
| 驗證接觸點 | `validateTouchpoint` | `validate_touchpoint` | 驗證接觸點設定 |
