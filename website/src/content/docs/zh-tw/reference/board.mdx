---
title: 資料看板
description: client.boards 的結構與方法參考 — CRM 管道、欄位、項目、區隔、匯入/匯出和 Knowledge Hub。
---
...
|--------|-----------|--------|------|
| 列出 boards | `list` | `list` | 列出所有 board |
| 取得 board | `get` | `get` | 依 ID 取得 board |
| 建立 board | `create` | `create` | 建立新 board |
| 更新 board | `update` | `update` | 更新 board 名稱/說明 |
| 刪除 board | `delete` | `delete` | 刪除 board |
| 重新排序 boards | `reorder` | `reorder` | 變更顯示順序 |

### 匯入 / 匯出

| 方法 | TypeScript | Python | 說明 |
|--------|-----------|--------|------|
| 匯出 CSV | `exportCsv` | `export_csv` | 將所有項目匯出為 CSV |
| 匯入 CSV | `importCsv` | `import_csv` | 從 CSV 檔案匯入項目 |
| 匯入 Excel | `importExcel` | `import_excel` | 從 Excel 檔案匯入項目 |
| 取得匯入進度 | `getImportProgress` | `get_import_progress` | 輪詢進行中匯入的狀態 |

### 欄位

| 方法 | TypeScript | Python | 說明 |
|--------|-----------|--------|------|
| 建立欄位 | `createField` | `create_field` | 向 board 新增欄位 |
| 更新欄位 | `updateField` | `update_field` | 修改欄位設定 |
| 刪除欄位 | `deleteField` | `delete_field` | 移除欄位 |
| 重新排序欄位 | `reorderFields` | `reorder_fields` | 變更欄位順序 |
| 批次更新欄位 | `bulkUpdateFields` | `bulk_update_fields` | 同時更新多個欄位 |

### 項目

| 方法 | TypeScript | Python | 說明 |
|--------|-----------|--------|------|
| 列出項目 | `listItems` | `list_items` | 分頁的 board 項目列表 |
| 取得項目 | `getItem` | `get_item` | 取得單一項目 |
| 建立項目 | `createItem` | `create_item` | 向 board 新增項目 |
| 更新項目 | `updateItem` | `update_item` | 更新項目欄位值 |
| 刪除項目 | `deleteItem` | `delete_item` | 移除項目 |
| 批次刪除 | `bulkDeleteItems` | `bulk_delete_items` | 依 ID 刪除多個項目 |
| 搜尋 | `search` | `search` | 在 board 內進行全文搜尋 |
| 連結項目 | `linkItems` | `link_items` | 將項目連結到相關 board 項目 |
| 取消連結項目 | `unlinkItems` | `unlink_items` | 移除項目間的連結 |

### 區隔

| 方法 | TypeScript | Python | 說明 |
|--------|-----------|--------|------|
| 列出區隔 | `listSegments` | `list_segments` | 列出已儲存的篩選檢視 |
| 建立區隔 | `createSegment` | `create_segment` | 將篩選條件儲存為區隔 |
| 更新區隔 | `updateSegment` | `update_segment` | 更新區隔的篩選條件 |
| 刪除區隔 | `deleteSegment` | `delete_segment` | 移除區隔 |
