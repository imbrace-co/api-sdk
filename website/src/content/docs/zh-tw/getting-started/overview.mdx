...
- **AI Agents** — 串流以您的知識庫訓練的 AI agent 的即時回應
...
- **存取權杖** — 用於面向使用者的應用程式。使用者在透過 OTP 或 OAuth 登入後取得。在初始化用戶端時作為 `accessToken` 傳遞。
- **API 金鑰** — 用於伺服器對伺服器整合。從 Imbrace 管理儀表板產生一個。在初始化用戶端時作為 `apiKey` 傳遞。
