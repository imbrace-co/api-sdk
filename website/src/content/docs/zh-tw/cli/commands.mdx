...
`create` 和 `update` 接受相同的標誌。`update` 透過 PUT-merge 保留未變更的欄位。`--id-only` 僅適用於 `create`。
...
`imbrace mcp` 透過 **stdio** 執行一個 [Model Context Protocol](https://modelcontextprotocol.io) 伺服器，把 Imbrace API 作為工具提供給 AI agent（Claude Code、Claude Desktop、Cursor 等）。工具與 SDK 的 [`client.api`](/zh-tw/sdk/overview/) 介面出自同一套 `agent-tools` OpenAPI 規格 — 5 個服務共 272 個操作 — 命名格式為 `<service>_<operationId>`，例如 `dataBoard_listBoards`、`workflow_listFlows`。
...
管理多組憑證 profile（AWS 風格）。每個 profile 會儲存憑證、環境預設、組織 ID 與其他 SDK 設定。解析順序：`--profile` 標誌 > `IMBRACE_PROFILE` 環境變數 > `active_profile` 設定 > `"default"`。
