---
title: CLI 指令
description: 所有 Imbrace CLI 指令的完整參考。
---
...
一個 AI agent 是一個設定好的助理（LLM + 提示詞 + 行為）。建立 agent 時會以單一原子操作佈建助理、網頁頻道和使用案例範本。
...
`create` 和 `update` 接受相同的行為標誌。`update` 透過 PUT-merge 保留未變更的欄位。`--id-only` 僅適用於 `create`。

| 標誌 | 對應至 | 備註 |
|---|---|---|
| **身份** | | |
| `--name` / `-n` | `name` + `title` | 建立時必填 |
| `--description` / `-d` | `description` + `short_description` | 在 UI 中顯示於標題下方 |
...
| **輸出** | | |
| `--json` | — | 機器可讀輸出 |
| `--id-only` | — | 僅列印新 agent ID（僅限 create） |
...
| UI 分類 | API 資料夾名稱 |
|---|---|
| 頻道工作流程 | `Channel Workflow` |
| Board 自動化 | `Board Automation` |
| AI Agent 技能 | `AI Agent Capabilities` |
| 其他 | `Others` |
...
> `@imbrace/cli` `v0.8.0`+。請勿與上文的 `imbrace workflow mcp ...` 混淆 — 那組指令管理的是工作流程引擎自身的 MCP-server 資源。想零安裝？閘道透過 HTTP 託管同一套工具 — 請參閱 [MCP Server](/zh-tw/mcp/overview/)。

`imbrace mcp` 透過 **stdio** 執行一個 [Model Context Protocol](https://modelcontextprotocol.io) 伺服器，把 Imbrace API 作為工具提供給 AI agent（Claude Code、Claude Desktop、Cursor 等）。工具與 SDK 的 [`client.api`](/sdk/overview/#the-full-api-surface--clientapi-typescript) 介面出自同一套 `agent-tools` OpenAPI 規格 — 5 個服務共 272 個操作 — 命名格式為 `<service>_<operationId>`，例如 `dataBoard_listBoards`、`workflow_listFlows`。

```bash
imbrace mcp                                     # 唯讀工具（預設）
imbrace mcp --allow-write                       # + 建立/修改資料的操作
imbrace mcp --allow-write --allow-destructive   # + 刪除資料的操作
imbrace mcp --services dataBoard,workflow       # 限定服務
imbrace mcp --tags boards,records               # 依 OpenAPI 標籤限定
imbrace mcp --profile dev                       # 以指定 profile 執行
```
...
| 預設（唯讀） | 140 |
| `--allow-write` | 229 |
| `--allow-destructive`（隱含 `--allow-write`） | 272 |

服務：`dataBoard`、`channel`、`platform`、`marketplace`、`workflow`。

**請先登入。** stdout 是 JSON-RPC 通道，因此指令會直接回報錯誤而不是互動式提示：
...
管理多組憑證 profile（AWS 風格）。每個 profile 會儲存憑證、環境預設集（preset）、組織 ID 與其他 SDK 設定。解析順序：`--profile` 標誌 > `IMBRACE_PROFILE` 環境變數 > `active_profile` 設定 > `"default"`。

### 建立

建立新的 profile，並向平台驗證憑證：
...
`--force` 會略過憑證驗證。若 profile 已存在，指令會回報錯誤。
...
顯示所有 profile，並標示使用中的 profile、環境與狀態。
...
預設為使用中的 profile。顯示所有已儲存的欄位（憑證預覽、方式、電子郵件、環境、base URL、組織 ID、逾時、服務、健康檢查）。
...
將 CLI 自我更新至 npm 上最新發佈的版本。

```bash
imbrace update              # 更新至最新版本
imbrace update --check      # 檢查更新但不安裝
imbrace update --version 0.6.5  # 安裝指定版本
imbrace update --json       # 機器可讀的 JSON 輸出
```
...
<Aside type="note">
  `imbrace update` 從 **v0.6.8** 起可用。舊版本使用者需先手動執行 `npm install -g @imbrace/cli@latest` 一次以取得此指令。
</Aside>
...
8. **建立時的 `--no-use-memory`** — 可能不會生效。請在建立後再更新。
