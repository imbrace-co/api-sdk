...
2. **Create an AI agent**
   ...
   <Aside type="caution" title="Always pass streaming: true for chat UIs">
     Newly-created agents default to **`streaming: false`** on the server. With that default, `streamChat` returns the full reply as a single `text/plain` response — no `text-delta` events — so the `streamChatText` helper (and any other SSE parser) yields **zero chunks** and a chat UI shows nothing. Passing `streaming: true` at creation time makes the server emit the Vercel-AI-SDK UI-Message-Stream (`Content-Type: text/event-stream`) that `streamChatText` is designed to parse. You can also flip it later via `updateAiAgent(id, { streaming: true })`.
   </Aside>
...
3. **Stream a chat response using the AI agent**
   ...
   <Aside type="tip" title="Working with a non-streaming agent?">
     If the agent has `streaming: false`, the server returns the full reply as one `text/plain` response — no `text-delta` events for `streamChatText` to parse, so it yields nothing. The pattern below handles both shapes by branching on `Content-Type`:
...
3. **Attach the folder to the AI agent**
   ...
   Pass the folder `_id` in `folder_ids` — the AI agent retrieves from every file in that folder. Use `board_ids` to attach a CRM data-board (its items become a knowledge source — see [Resources → Boards & items](/sdk/databoard/#items)). The legacy `knowledge_hubs` field is deprecated.
