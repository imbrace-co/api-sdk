...
The same operation registry powers the gateway's [hosted MCP server](/mcp/overview/) and the CLI's [`imbrace mcp`](/cli/commands/#mcp-server) command, both of which expose these operations as MCP tools for AI agents.
