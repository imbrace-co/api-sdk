...
description: Cài đặt Imbrace SDK, thiết lập môi trường, cấu hình thông tin xác thực và ghi đè URL dịch vụ.
---

import { Tabs, TabItem, Aside } from "@astrojs/starlight/components";

Hướng dẫn này bao gồm mọi thứ bạn cần để đi từ số không đến một tích hợp Imbrace SDK hoạt động — cài đặt, môi trường, thông tin xác thực và ghi đè URL dịch vụ.
...
# Đăng nhập bằng mật khẩu (POST /platform/v1/login/authenticate) — phổ biến nhất cho ứng dụng end-user.
# login() trả về danh sách tổ chức; chọn 1 rồi select_organization() đổi token
# login_acc_ ngắn hạn lấy token acc_ gắn theo tổ chức.
...
Xem [Cài đặt CLI](/cli/installation/) để biết các phương pháp cài đặt thay thế và [Lệnh CLI](/cli/commands/) để biết tài liệu tham khảo lệnh đầy đủ.
...
| `platform`           | `platform`          | Dịch vụ Platform                    |
| `channel_service`    | `channelService`    | Dịch vụ Channel                     |
| `data_board`         | `dataBoard`         | Data Board                          |
| `backend`            | `backend`           | Monolith cũ (`/v1/backend`) — dùng cho đăng nhập xác thực, template và một số route chưa được tách ra |
| `ips`                | `ips`               | Dịch vụ IPS                         |
| `ai`                 | `ai`                | Dịch vụ AI                          |
| `marketplaces`       | `marketplaces`      | Dịch vụ Marketplace                 |
| `file_service`       | `fileService`       | Dịch vụ File                        |
| `message_suggestion` | `messageSuggestion` | Dịch vụ Message Suggestion          |
| `predict`            | `predict`           | Dịch vụ Predict                     |
| `workflow_engine`    | `workflowEngine`    | Workflows            |
| `ai_agent`           | `aiAgent`           | Dịch vụ AI Agent                    |
...
API Key đã hết hạn hoặc không hợp lệ. Tạo key mới từ [Imbrace Dashboard](https://cloud.imbrace.co) hoặc xem [hướng dẫn API Key](/guides/api-key).
