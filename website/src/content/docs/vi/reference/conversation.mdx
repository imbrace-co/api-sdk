---
title: Hội thoại
description: Tham chiếu schema và method cho client.conversations — liệt kê, tìm kiếm, tham gia, phân công và quản lý luồng hội thoại.
---

import { Tabs, TabItem } from "@astrojs/starlight/components";

`client.conversations` quản lý các luồng hội thoại giữa các liên hệ và nhóm của bạn (hoặc AI agent). Mỗi hội thoại thuộc về một [Kênh](/reference/channel/) và được liên kết với một [Liên hệ](/reference/contact/).
...
| `channel_type` | string | Loại nền tảng kênh (xem [Kênh](/reference/channel/)) |
| `contact_id` | string | ID liên hệ được liên kết |
...
|--------|-----------|--------|-------|
| Danh sách | `list` | `list` | Danh sách hội thoại có phân trang |
| Lấy | `get` | `get` | Lấy hội thoại theo ID nội bộ |
| Lấy theo ID hội thoại | `getByConversationId` | — | Lấy theo ID hội thoại công khai |
| Tìm kiếm | `search` | `search` | Tìm kiếm toàn văn trong đơn vị kinh doanh |
| Đếm theo view | `getViewsCount` | `get_views_count` | Đếm hội thoại theo view/trạng thái |
| Tạo | `create` | `create` | Tạo hội thoại mới |
| Tham gia | `join` | `join` | Tham gia hội thoại với tư cách thành viên nhóm |
| Rời | `leave` | `leave` | Rời khỏi hội thoại |
| Cập nhật trạng thái | `updateStatus` | `update_status` | Thay đổi trạng thái hội thoại |
| Cập nhật tên | `updateName` | — | Đổi tên hội thoại |
| Bắt đầu cuộc gọi video | `initVideoCall` | — | Bắt đầu cuộc gọi video trong hội thoại |
| Gán thành viên nhóm | `assignTeamMember` | — | Gán thành viên nhóm vào hội thoại |
| Xóa thành viên nhóm | `removeTeamMember` | — | Xóa thành viên nhóm |
| Lấy người dùng có thể mời | `getInvitableUsers` | — | Danh sách người dùng có thể mời |
