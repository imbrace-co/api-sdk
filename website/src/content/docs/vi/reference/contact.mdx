...
|--------|-----------|--------|-------|
| Danh sách | `list` | `list` | Danh sách contacts có phân trang |
| Lấy | `get` | `get` | Lấy contact theo ID |
| Cập nhật | `update` | `update` | Cập nhật trường contact |
| Tìm kiếm | `search` | `search` | Tìm kiếm toàn văn contacts |
| Xuất CSV | `exportCsv` | `export_csv` | Tải xuống tất cả contacts dưới dạng CSV |
| Lấy hội thoại | `getConversations` | `get_conversations` | Danh sách hội thoại của contact |
| Lấy comment | `getComments` | `get_comments` | Danh sách comments nội bộ trên contact |
| Lấy file | `getFiles` | `get_files` | Danh sách files đính kèm với contact |
| Lấy hoạt động | `getActivities` | `get_activities` | Nhật ký hoạt động hội thoại |
| Tải lên avatar | `uploadAvatar` | `upload_contacts` | Tải lên file cho contacts (avatar / import file) |
