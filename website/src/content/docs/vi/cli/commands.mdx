---
title: Lệnh CLI
description: Tham chiếu đầy đủ cho tất cả lệnh Imbrace CLI.
---
...
`create` và `update` chấp nhận cùng các cờ hành vi. `update` giữ nguyên các trường không thay đổi thông qua PUT-merge. `--id-only` chỉ có trên `create`.
...
| **Đầu ra** | | |
| `--json` | — | Đầu ra dạng máy đọc được |
| `--id-only` | — | Chỉ in ID agent mới (chỉ khi tạo) |
...
`get` lấy trợ lý bên dưới để `sub_agents` / `team_leads` hiển thị chính xác (chúng nằm trên trợ lý, không phải use case).
...
> `@imbrace/cli` `v0.8.0`+. Đừng nhầm với `imbrace workflow mcp ...` ở trên — nhóm lệnh đó quản lý tài nguyên MCP-server của chính workflow engine. Muốn không phải cài gì? Gateway host sẵn cùng bộ tool này qua HTTP — xem [MCP Server](/mcp/overview/).

`imbrace mcp` chạy một server [Model Context Protocol](https://modelcontextprotocol.io) qua **stdio**, mở API Imbrace thành tool cho AI agent (Claude Code, Claude Desktop, Cursor, …). Tool được sinh từ cùng bộ spec OpenAPI `agent-tools` với bề mặt [`client.api`](/sdk/overview/#the-full-api-surface--clientapi-typescript) của SDK — 272 operation trên 5 service — và đặt tên dạng `<service>_<operationId>`, ví dụ `dataBoard_listBoards`, `workflow_listFlows`.

```bash
imbrace mcp                                     # tool chỉ đọc (mặc định)
imbrace mcp --allow-write                       # + operation tạo/sửa dữ liệu
imbrace mcp --allow-write --allow-destructive   # + operation xóa dữ liệu
imbrace mcp --services dataBoard,workflow       # giới hạn theo service
imbrace mcp --tags boards,records               # giới hạn theo tag OpenAPI
imbrace mcp --profile dev                       # chạy với một profile cụ thể
```

| Chế độ | Số tool |
|---|---|
| mặc định (chỉ đọc) | 140 |
| `--allow-write` | 229 |
| `--allow-destructive` (bao hàm `--allow-write`) | 272 |

Service: `dataBoard`, `channel`, `platform`, `marketplace`, `workflow`.

**Đăng nhập trước.** stdout là kênh JSON-RPC nên lệnh sẽ báo lỗi ngay thay vì hỏi tương tác:
...
```bash
claude mcp add imbrace -- imbrace mcp --services dataBoard,workflow
```

Hoặc cấu hình cho bất kỳ MCP client nào qua JSON:

```json
{
  "mcpServers": {
    "imbrace": { "command": "imbrace", "args": ["mcp", "--allow-write"] }
  }
}
```
...
Quản lý nhiều profile thông tin xác thực (theo kiểu AWS). Mỗi profile lưu thông tin xác thực, preset môi trường, ID tổ chức và các thiết lập SDK khác. Thứ tự ưu tiên: cờ `--profile` > biến môi trường `IMBRACE_PROFILE` > cấu hình `active_profile` > `"default"`.

### Tạo

Tạo một profile mới và xác minh thông tin xác thực với nền tảng:
...
`--force` bỏ qua bước xác minh thông tin xác thực. Nếu profile đã tồn tại, lệnh sẽ báo lỗi.
...
Hiển thị tất cả profile kèm dấu đánh dấu profile đang hoạt động, môi trường và trạng thái.
...
Mặc định là profile đang hoạt động. Hiển thị tất cả các trường đã lưu (bản xem trước thông tin xác thực, phương thức, email, môi trường, base URL, ID tổ chức, timeout, services, health check).

### Sử dụng

Chuyển profile đang hoạt động cho tất cả các lệnh tiếp theo:
...
Giữ nguyên thông tin xác thực và trạng thái hoạt động.
...
Không ảnh hưởng đến dữ liệu trên nền tảng — chỉ xóa bộ thông tin xác thực cục bộ.
...
Tự cập nhật CLI lên phiên bản mới nhất trên npm.

```bash
imbrace update              # cập nhật lên phiên bản mới nhất
imbrace update --check      # kiểm tra cập nhật mà không cài đặt
imbrace update --version 0.6.5  # cài đặt phiên bản cụ thể
imbrace update --json       # đầu ra dạng JSON
```
...
<Aside type="note">
  `imbrace update` khả dụng từ **v0.6.8** trở đi. Khách hàng dùng phiên bản cũ hơn cần chạy `npm install -g @imbrace/cli@latest` một lần để có lệnh này.
</Aside>
