...
`create` và `update` chấp nhận cùng các cờ. `update` giữ nguyên các trường không thay đổi thông qua PUT-merge. `--id-only` chỉ có trên `create`.
...
Mặc định là profile đang hoạt động. Hiển thị tất cả các trường đã lưu (bản xem trước credential, phương thức, email, môi trường, base URL, ID tổ chức, timeout, services, health check).
