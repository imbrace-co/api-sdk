...
try {
  ...
  if (e instanceof ApiError) {
    ...
    // ví dụ: "HTTP 404: Contact not found"
  }
}
```
...
try:
    ...
    print(f"Lỗi mạng: {e}")
    # ví dụ: "Network error or timeout: ConnectTimeout(...)"
```
</TabItem>
</Tabs>
...
**Backoff:** `2^retryCount` giây giữa các lần thử (2s → 4s). Trường hợp xấu nhất: tổng cộng 3 lần thử.
</TabItem>
<TabItem label="Python">
| Điều kiện                      | Hành động                                |
| ------------------------------ | ---------------------------------------- |
...
**Backoff:** `2^retryCount` giây giữa các lần thử (1s → 2s → 4s). Trường hợp xấu nhất: tổng cộng 4 lần thử.
</TabItem>
</Tabs>
...
<TabItem label="Python">
```python
# 1. Sử dụng context manager để kết nối luôn được đóng một cách chắc chắn
...
try:
    ...
    if e.status_code == 409:
        print("Xung đột — liên hệ đã được sửa đổi")
