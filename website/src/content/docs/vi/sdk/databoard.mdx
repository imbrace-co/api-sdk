---
title: Data Boards
description: Tài liệu tham khảo SDK cho client.boards — CRM pipelines, items, fields, segments, Knowledge Hub, import/export, và tích hợp ổ đĩa ngoài.
---

import { Tabs, TabItem } from "@astrojs/starlight/components";

Boards là kho dữ liệu cốt lõi cho các pipeline CRM — khách hàng tiềm năng, giao dịch, nhiệm vụ, hoặc bất kỳ dữ liệu có cấu trúc nào. Truyền ID board trong `board_ids` khi tạo AI agent để cấp cho nó quyền truy cập vào dữ liệu đó — xem [Hướng dẫn toàn diện §4](/sdk/full-flow-guide/#4-manage-data-boards-and-items-crm-pipelines).
...
# create_item sử dụng {"fields": [{"board_field_id", "value"}]}
...
# update_item sử dụng {"data": [{"key", "value"}]} — lưu ý "key", không phải "board_field_id"
...
Xung đột giờ được trả về trực tiếp: `updateItem` trả `HTTP 409 Conflict` khi bản ghi đã bị sửa kể từ lần đọc cuối — bắt `ApiError` (`statusCode === 409`) thay vì gọi endpoint riêng.
...
// Liên kết và hủy liên kết (data-board — POST/DELETE /boards/:id/items/:itemId/related)
