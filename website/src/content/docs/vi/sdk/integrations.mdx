...
<Aside type="tip">
  Đừng chia sẻ một instance `ImbraceClient` duy nhất giữa các Celery worker — hãy tạo một client mới cho mỗi lần gọi tác vụ bằng context manager. Pool kết nối httpx không an toàn để chia sẻ giữa các tiến trình.
</Aside>
...
- [Xác thực →](/sdk/authentication/) — luồng đăng nhập OTP, vòng đời thông tin xác thực và lưu trữ token
- [Xử lý lỗi →](/sdk/error-handling/) — cách `AuthError`, `ApiError` và `NetworkError` được phát sinh
- [Bắt đầu nhanh →](/sdk/quick-start/) — ví dụ tối thiểu chạy được trước khi kết nối với framework
