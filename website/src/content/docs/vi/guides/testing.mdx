...
Lệnh này chạy vitest trên toàn bộ dự án (unit + bất kỳ tệp cục bộ nào khớp với mẫu kiểm thử). TypeScript integration tests chưa có sẵn — xem `py/tests/integration/` để kiểm tra API trực tiếp, hoặc `test/test-local-pkg/ts/` để có các script kiểm thử trực tiếp thủ công.
