...
**Vibe Coding** có nghĩa là viết mã bằng cách cộng tác với trợ lý AI — mô tả những gì bạn muốn bằng ngôn ngữ tự nhiên và để AI tạo ra, giải thích hoặc tái cấu trúc mã cho bạn. Imbrace SDK đi kèm với tệp [`llms.txt`](https://developer.imbrace.co/llms.txt) để bất kỳ công cụ AI nào cũng có thể hiểu ngay SDK mà không bịa ra tên phương thức hoặc cấu trúc tham số.
...
Các lệnh resource (`data-board`, `ai-agent`, `workflow`, `document-ai`, `orchestrator`, `guardrail`) hỗ trợ `--json` để có đầu ra dạng máy đọc được. Mọi lệnh đều hỗ trợ `-h` / `--help` để xem hướng dẫn sử dụng. Xem [Lệnh CLI](/cli/commands/) để biết tài liệu tham khảo đầy đủ.
