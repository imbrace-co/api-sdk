...
Self-update the CLI to the latest published version on npm.

```bash
imbrace update              # update to latest
imbrace update --check      # check for updates without installing
imbrace update --version 0.6.5  # install a specific version
imbrace update --json       # machine-readable output
```
...
> **Note:** `imbrace update` is available from v0.6.8 onwards. Customers on older versions must run `npm install -g @imbrace/cli@latest` manually for the first upgrade.
