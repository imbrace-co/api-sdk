...
2. **Create an AI agent**
   ...
   **TypeScript**
   ...
   const agent = await client.chatAi.createAiAgent({
     ...
     streaming: true,         // emit text-delta SSE events (required for chat UIs)
   })
...
> **Always pass `streaming: true` for chat UIs.** Newly-created agents default to `streaming: false` on the server. With that default, `/v2/chat` returns the full reply as a single `text/plain` response (no `text-delta` events), so `streamChatText` and any other SSE parser yields **zero chunks** and a chat UI shows nothing. Passing `streaming: true` at creation time makes the server emit the Vercel-AI-SDK UI-Message-Stream (`Content-Type: text/event-stream`). You can also flip it later via `updateAiAgent(id, { streaming: true })`.

3. **Stream a chat response using the AI agent**

   Use `streamChatText` / `stream_chat_text` — it wraps `streamChat`, parses the SSE event stream, and yields plain text chunks. Assumes the agent was created with `streaming: true` (see the callout in step 2).
   ...
   **Python**
   ...
   > **Working with a non-streaming agent?** If the agent has `streaming: false`, the server returns the full reply as one `text/plain` response — no `text-delta` events for `streamChatText` to parse, so it yields nothing. The pattern below branches on `Content-Type` to handle both shapes:
   >
   > ```typescript
   > const res = await client.aiAgent.streamChat({ assistant_id: aiAgentId, messages: [...] })
   ...
   >   }
   > } else {
   >   process.stdout.write(await res.text())   // non-streaming agent
   > }
   > ```
