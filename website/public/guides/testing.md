...
This runs vitest across the whole project (unit + any local files matching test patterns). TypeScript integration tests aren't available yet — see `py/tests/integration/` for live API checks, or `test/test-local-pkg/ts/` for manual live testing scripts.
